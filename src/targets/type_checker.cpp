...
void og::type_checker::do_BooleanLogicalExpression(cdk::binary_operation_node * const node, int lvl) {
  ...
  if (node->left()->type()->name() != cdk::TYPE_INT) throw std::string("integer expression expected in bnary expression");

  node->right()->accept(this, lvl + 2);
  if (node->right()->type()->name() != cdk::TYPE_INT) throw std::string("integer expression expected in bnary expression");
